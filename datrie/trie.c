                          ...
                          Bool             is_overwrite)
{
    ...
    /* 找到根节点的索引为2 */
    s = da_get_root (trie->da);
    /* 从模式串开始，trie_da_is_separate表示本节点后有一个后缀 */
    for (p = key; !trie_da_is_separate (trie->da, s); p++)
    {
        /* 将输入的字符进行转换 */
        TrieIndex tc = alpha_map_char_to_trie (trie->alpha_map, *p);
    ...
    /* walk through tail */
    /* 走到这里说明整个字符串中已经存在的都插入了，剩下的遇到了后缀 */
    sep = p;
    /* 获取该状态的后缀 */
                       ...
                       TrieData        data)
{
    ...
    /* 插入老后缀的剩余字符的第一个字符，其余的当做后缀，放入后缀数组 */
